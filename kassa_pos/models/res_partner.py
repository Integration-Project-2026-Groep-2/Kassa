...
class ResPartner(models.Model):
    ...
    def action_generate_qrcode(self):
        """Generate QR code from badge_code and show in popup."""
        self.ensure_one()
        
        if not self.badge_code:
            raise ValueError('Badge code is required to generate QR code')
        
        # Generate QR code
        qr_image = self._generate_qr_code(self.badge_code)
        
        # Create a transient record to hold the QR code data
        wizard = self.env['res.partner.qrcode.wizard'].create({
            'partner_id': self.id,
        ...
        return {
            ...
            'res_model': 'res.partner.qrcode.wizard',
            'res_id': wizard.id,
            'view_mode': 'form',
            'view_type': 'form',
            'target': 'new',
    ...
    def _generate_qr_code(data):
        """Generate QR code image from badge code."""
        try:
            import qrcode
            import io
            import base64
